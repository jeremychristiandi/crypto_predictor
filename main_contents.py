...
def predict_contents(df, ticker, periods, is_predict=False):
    # 1. Menyisakan kolom date dan price
    if is_predict:
        df_date_price = pd.DataFrame(df[["Date","Close"]])
        df_date_price = df_date_price.rename(columns={"Close": "Price"})
    ...
    with st.spinner("Waiting for prediction..."):
        lstm_error, lstm_timesteps, lstm_data, lstm_data_actual = create_lstm_prediction(df_date_price, ticker, periods, is_predict=is_predict)
        nbeats_error, nbeats_timesteps, nbeats_data, nbeats_data_actual = create_nbeats_prediction(df_date_price, ticker, periods, is_predict=is_predict)
...
def visualize_pred_actual(timesteps, data, data_actual, start=0, end=None, model_name="LSTM", errors=None, is_predict=False):
    ...
    fig = go.Figure()
    if not is_predict:
        fig.add_trace(go.Scatter(x=data_timesteps, y=actual_prices, name="Actual", marker=dict(color='blue'), mode="markers"))
    fig.add_trace(go.Scatter(x=data_timesteps, y=pred_prices, name="Prediction", marker=dict(color='orange')))
...
def create_lstm_prediction(df_date_price, ticker, periods, is_predict=False):
    ...
    # Selected page is predict
    if is_predict:
        lstm_pred = create_prediction(lstm_model, windows_test[-periods:])
    # Selected page is homepage
    else:
        lstm_pred = create_prediction(lstm_model, windows_test[:periods])
    ...
    # initializing date
    test_date = df_date_price.index.values[-1]

    date_generated = pd.date_range(test_date, periods=periods+1)
    date_generated = date_generated[1:]

    if is_predict:
        return lstm_res, date_generated, data, data_actual
...
def create_nbeats_prediction(df_date_price, ticker, periods, is_predict=False):  
    options = model_options(ticker)
    windows = options['windows']
    horizon = options['horizon']
    batch_size = options['batch_size']
    ...
    if os.path.exists(NBEATS_PATH):
        nbeats_model = load_model(NBEATS_PATH, custom_objects={"NBeatsBlock": NBeatsBlock})
    else:   
        # Pembuatan Model
        tf.random.set_seed(42)

        block_layer = NBeatsBlock(INPUT_SIZE,
    ...
    if is_predict:
        return nbeats_res, date_generated, data, data_actual
